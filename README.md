...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/Fork_Version-5.19.2--bas.4-6f42c1" alt="Fork version 5.19.2-bas.4">
...
Собственная версия форка — **`v5.19.2-bas.4`**. Формат `<upstream-sync>-bas.<revision>` сохраняет связь с оригиналом: `5.19.2` — текущий полный upstream sync marker, а `bas.4` — очередная fork-ревизия после синхронизации. История существующих коммитов и их даты не переписываются.
...
**Правило версий:** fork-only релизы увеличивают `bas.N` (`5.19.2-bas.4`), а после перехода на новый upstream sync marker нумерация начинается заново. Старые коммиты и их даты не переписываются; версия указывается только в новых релизных коммитах и тегах.

> Команды установки ниже идут напрямую из ветки `main`. Текущий sync marker — upstream `v5.19.2`, версия форка — `v5.19.2-bas.4`.
