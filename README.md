...
* **Веб-панель:** HTTPS `:8443`, self-signed TLS, bearer token, список клиентов, добавление/удаление, QR/config/vpnuri, статистика, логи, рестарт сервиса.
* **Новые команды управления:** `p2p list/show/add/remove`, `ipv6 status/upgrade`.
* **Автогенерация firewall hooks:** `/root/awg/postup.sh`, `/root/awg/postdown.sh`, `/root/awg/p2p_rules.sh`.
* **План развития:** AdGuard Home как DNS-фильтр для блокировки рекламы, трекеров, telemetry/malware и мусорных DNS-запросов.
...
* В `/root/awg/awgsetup_cfg.init` добавлены ключи `AWG_IPV6_*`, `AWG_P2P_*`, `AWG_FULLCONE_NAT`, `AWG_WEB_*`.
...
Следующий крупный блок для этого форка — встроенная установка **AdGuard Home** как DNS-сервера для VPN-клиентов.
...
* добавить installer-флаги `--enable-adguard`, `--adguard-port=3000`, `--dns-mode=adguard|system|custom`;
* ставить AdGuard Home в `/opt/AdGuardHome` или `/root/awg/adguard` без Docker;
* слушать DNS на VPN-интерфейсе `awg0`, чтобы DNS не становился открытым публичным resolver-ом;
* автоматически прописывать DNS клиентов как адрес сервера в туннеле: `10.9.9.1` и, при IPv6, серверный IPv6 из `AWG_IPV6_SUBNET`;
* открывать в firewall только нужные порты: DNS внутри VPN, web-admin AdGuard либо только localhost/VPN, либо явно выбранный bind;
* добавить управление через `manage_amneziawg.sh dns status|restart|logs|set-mode`;
* добавить карточку DNS/AdGuard в веб-панель: статус, ссылки, выбранный режим, последние события;
* включить базовые списки фильтров против рекламы, трекеров, malware/phishing и telemetry, но оставить возможность отключить агрессивные списки;
* добавить миграцию существующих клиентов через `regen`, чтобы новые DNS попали в `.conf`, QR и `vpn://`.
