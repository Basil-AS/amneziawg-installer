...
`--preset=mobile` рассчитан на LTE/5G, CGNAT и нестабильный роуминг: используются умеренные размеры junk/padding, непересекающиеся H-диапазоны, `S4 >= 12` и `KeepaliveTimeout` 25-35 секунд для AWG 3.1.
