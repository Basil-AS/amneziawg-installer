...
Для установки без вопросов передайте `--yes` и режим маршрутизации, например `sudo bash ./install_amneziawg.sh --yes --route-amnezia`.

**Режимы маршрутизации** (что пойдёт в туннель):

| Флаг | Режим | Когда брать |
|---|---|---|
| `--route-amnezia` | «Amnezia», список подсетей | по умолчанию, российские сайты идут мимо туннеля |
| `--route-all` | «Весь трафик» | когда нужен полный туннель |
| `--route-custom=СЕТИ` | «Пользовательский» | когда список подсетей свой |

⚠️ Если в приложении Amnezia не открывается раздельное туннелирование по сайтам или Linux-клиент зацикливает маршруты, переустановите с `--route-all`: приложению для своей функции нужен именно полный туннель. Разбор: [ADVANCED, AllowedIPs](ADVANCED.md#allowedips-adv).

Часто используемые флаги: `--port=39743` (любой порт 1-65535), `--subnet=10.9.9.1/24`, `--disallow-ipv6`, `--allow-ipv6-tunnel`, `--preset=balanced|mobile|stealth|compatibility`, `--mobile` (пресет для мобильных сетей плюс порт 443/udp одним флагом), `--isolation=on|off` (изоляция клиентов друг от друга, по умолчанию включена), `--endpoint=ПУБЛИЧНЫЙ_IP` (нужен, когда внешний адрес сервера отличается от адреса на интерфейсе: типично для Oracle Cloud, GCP и любого NAT). Полный список: `--help` или [ADVANCED, CLI установщика](ADVANCED.md#install-cli-adv).
