...
independent per-client IP-family permission model is not silently copied:
implementing it requires a separate firewall/state-machine design so it cannot
weaken existing IPv6 leak-block, P2P/DNAT or client-isolation guarantees.

## Release policy

The current release is `5.29.0-bas.5`, retaining the upstream base number and
