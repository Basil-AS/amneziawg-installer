...
| `35f516c` plus its tests | Build-id regression tests | Ported | Tests cover the new diagnostic independently and do not alter protocol selection. |
...
The duplicated `35f516c` row above is kept as a test-scope note: the source
commit itself is one logical change, while its independent test coverage is
called out because it is part of the safety decision.
