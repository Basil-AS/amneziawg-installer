...
Fork releases use `<upstream-sync>-bas.<revision>`. Current fork version is `5.15.3-bas.2`; `bas.N` increments for fork-only releases and resets to `bas.1` when the upstream sync marker changes. The marker does not imply a full upstream merge.
