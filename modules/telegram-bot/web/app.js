(() => {
  ...
  async function runNettest(button) {
    ...
    dialog.innerHTML = `<button class="dialog-close">×</button><h3>🧪 Тест скорости · ${esc(server)}</h3><p class="test-status">Подготовка…</p><p class="test-source">Точка измерения: защищённый Mini App gateway</p><div class="test-meter"><i></i></div><div class="result-grid test-values"><span>Ping API <b>—</b></span><span>Download <b>—</b></span><span>Upload <b>—</b></span><span>Статус <b>выполняется</b></span></div><p class="test-assessment"></p><button class="test-cancel">Отменить</button>`;
    ...
    try {
      ...
      stage('Сохранение отчёта…', 88); const report = await json(await request(`/api/nettest?server=${encodeURIComponent(server)}&kind=report&test_id=${encodeURIComponent(testId)}`, {method:'POST', body:JSON.stringify({network_type:'home', duration_seconds:Math.max(1, Math.round((performance.now() - pingStart) / 1000)), latency:{rtt_ms:pingMs}, download_probe:{mbps:downloadMbps, bytes:downloadBlob.size}, upload_probe:{mbps:uploadMbps, bytes:uploadBytes.byteLength}, user_agent:navigator.userAgent, comment:'Mini App gateway measurement'})})); stage('Готово', 100); values[3].textContent = 'готово'; dialog.querySelector('.test-assessment').textContent = report.report?.assessment?.summary || report.report?.assessment?.status || 'Отчёт сохранён на панели'; dialog.querySelector('.test-assessment').classList.add('visible'); dialog.querySelector('.test-cancel').textContent = 'Закрыть'; dialog.querySelector('.test-cancel').onclick = close; tg?.HapticFeedback?.notificationOccurred?.('success');
    } catch (error) {
