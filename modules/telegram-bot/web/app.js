(() => {
  ...
  async function clientAction(button) {
    ...
    button.disabled = true;
    try { const result = await api('/api/action', {method:'POST', body:JSON.stringify({server, action, name, ...(port === null ? {} : {port})})}); if (action === 'access-link' && result.url) { tg?.openLink?.(result.url); toast('Одноразовая ссылка открыта'); } else { toast(action === 'remove' ? 'Устройство удалено' : 'Настройка обновлена'); await load(); } }
    catch (error) { toast(error.message); } finally { button.disabled = false; }
  }
