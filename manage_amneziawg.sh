...
# после ошибочного значения и всё равно должен получить машинную ошибку.
if [[ -n "${_CLI_APPLY_MODE:-}" ]]; then
    case "$_CLI_APPLY_MODE" in
        syncconf|restart) export AWG_APPLY_MODE="$_CLI_APPLY_MODE" ;;
        *) _JSON_ERR="Недопустимое значение --apply-mode: '$_CLI_APPLY_MODE' (ожидается syncconf или restart)"; echo "$_JSON_ERR" >&2; exit 1 ;;
    esac
fi
...
# ==============================================================================
# Diagnose: read-only диагностика с опциональным сравнением по оператору
# ==============================================================================
    ...
    if command -v ufw &>/dev/null; then
        ...
        if [[ "$awg_port" -eq 0 ]]; then
            ...
            if ufw status 2>/dev/null | grep -qE "^${awg_port}/udp[[:space:]]+ALLOW"; then
                ...
                _diag_line WARN "UFW active, но ${awg_port}/udp не в ALLOW"
                warn=$((warn+1))
            fi
        else
    ...
    if [[ -n "$carrier" ]]; then
        ...
        if [[ -n "$jmax" && -n "$jmin" && "$jmax" =~ ^[0-9]+$ && "$jmin" =~ ^[0-9]+$ ]]; then
            ...
            if [[ "$jmax_off" -ge "$rc_jmax_off_lo" && "$jmax_off" -le "$rc_jmax_off_hi" ]]; then
                ...
                _diag_line WARN "Jmax-Jmin=$jmax_off вне [$rc_jmax_off_lo..$rc_jmax_off_hi]"
                warn=$((warn+1))
            fi
        else
        ...
        case "$rc_i1" in
            ...
            binary)
                if [[ -n "$i1" && "$i1" =~ ^\<r\ [0-9]+\>\<b\ 0x[0-9A-Fa-f]+\> ]]; then
                    ...
                    _diag_line WARN "I1=${i1:-absent}, $carrier требует binary I1 (<r N><b 0xHEX>)"
                    warn=$((warn+1))
                fi
                ;;
        esac
...
list_clients() {
    ...
    while IFS= read -r name; do
        ...
        local cf="?" png="?" pk="-" ip="-" ipv6="-" p2p="-" st="Нет данных" st_code="no_data"
        local color_start="" color_end=""
        if [[ "$NO_COLOR" -eq 0 ]]; then
            color_end="\033[0m"
...
usage() {
    ...
    echo "  geoip update-dbs      Скачать/обновить GeoIP MMDB базы (MaxMind GeoLite2, DB-IP)"
    echo "  geoip auto-update enable   Включить еженедельное автообновление GeoIP баз"
    ...
    echo "  web nginx-wait-awg0   Алиас для web fix-nginx-startup"
        ...
        case "$_sub" in
            update-dbs)
                if geoip_update_dbs; then
                    log "GeoIP базы обновлены."
                else
                    log_warn "Не удалось обновить одну или несколько GeoIP баз."
                    _cmd_rc=1
                fi
                ;;
            auto-update)
                ...
                case "$_au_sub" in
                    ...
                    status)
                        ...
                        ;;
                    *)
                        die "Неизвестная geoip auto-update команда: $_au_sub"
                        ;;
                esac
                ;;
            *)
                die "Неизвестная geoip команда: $_sub"
                ;;
        esac
        ;;
    ...
    web)
        ...
        case "$_sub" in
            ...
            reset-super)
                ...
                ;;
            check)
                [[ -z "${ARGS[2]:-}" ]] && die "Использование: web token check <token>"
            ...
            *)
                ...
                ;;
            *)
                die "Использование: web token list|create [--client NAME] [--name ALIAS]|add [ALIAS]|revoke <hash>|rotate <hash>|reset-super|check <token>|status|fix-nginx-startup|nginx-wait-awg0"
                ;;
        esac
        ;;
