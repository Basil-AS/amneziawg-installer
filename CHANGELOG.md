...
- Установщик и management-поверхность синхронизированы с upstream 5.28.1 без потери IPv6, P2P/DNAT, AdGuard Home, web-панели, Telegram и операторских пресетов форка.
- Добавлены защита критичных для загрузки пакетов, безопаснее обработка обновлений, проверка подписей релизов, структурированный JSON-вывод management-команд и дополнительные диагностики клиента/сервера.

### Исправлено

- Усилена обработка портов, endpoint, конфигов, restore и JSON против повреждённых или опасных значений из ручных правок.
- Сохранены IPv6-маршруты и leak-block, изоляция клиентов, имя сервера и регенерация клиентских артефактов в upgrade/restore-сценариях.

## [5.19.2-bas.9] - 2026-08-22

### Исправлено

- Панель остаётся доступной из VPN-туннеля; удалены ошибочные panel-port reject-правила.
- Генератор клиентских конфигов запрещает использовать домен веб-панели как VPN endpoint и сохраняет отдельный VPN endpoint/IP.
...
- Mini App сделан действительно опциональным: без `MINI_APP_URL` он не рекламируется Telegram и локальный gateway не запускается. Документация описывает кнопку «Панель», обязательный HTTPS reverse proxy и loopback-конфигурацию.
- Telegram-бот переработан в минимальный интерфейс: главное меню содержит только устройства, добавление, избранное, подключение, серверы и доступ; карточка устройства отделяет получение конфигурации от импорта и настроек. Нагрузка, готовность VPN, DNS/AdGuard, логи, сетевые тесты и обслуживание перенесены в веб-панель/Mini App.
- Telegram-бот: карточка «Клиенты» больше не печатает длинный список IP и P2P-портов; теперь это компактная сводка онлайн/всего с кнопкой перехода к устройствам.
- Telegram-бот: упорядочена навигация устройств — из списка убрана нерелевантная кнопка «Доступность», глобальная проверка панелей оставлена в главном меню, а внизу списка теперь только «Избранное/Добавить», пагинация и возврат в меню.
- Telegram-бот: владельцы scoped-токенов снова могут полноценно управлять своими конфигами — обновлять и удалять их, менять VPN/P2P/портовые правила, добавлять и удалять P2P-порты, а также получать свежие QR/`.conf`/URI и одноразовую ссылку импорта. Админское меню очищено от низкоуровневых kernel/resolver/token-диагностик.
- Проценты нагрузки и диагностики в веб-панели и Telegram-боте теперь всегда показываются с одним знаком после запятой; история считает начала отдельных событий, а не каждый повторный warning-сэмпл.
- Карточки здоровья веб-панели также округляют Load, RAM и Disk до одного знака; значения больше не выводятся с плавающей точностью JavaScript.
- AdGuard Home: добавлен API-прокси только через VPN-интерфейс для статуса, статистики, query log и управления URL-фильтрами; Telegram-администратор получил просмотр, обновление, добавление и удаление списков.
- Каталог клиентских приложений в Telegram объединяет одинаковые данные Finland/Germany в один экран, отключает превью ссылок и отдельно показывает Linux Desktop.
- Telegram-бот: кнопка «Обновить» редактирует текущий экран; устаревшие текстовые меню удаляются, а конфиги и медиафайлы сохраняются.
- Фоновые результаты диагностики теперь заменяют текущую карточку меню, а не создают второй навигационный экран.
- Администратор бота использует полный super-token для серверных операций, одновременно сохраняя отдельные scoped-токены пользователя для привязанных конфигов.
- Telegram-бот: исправлена обработка inline callback-кнопок, добавлена безопасная маршрутизация `nav:`-действий и отдельное меню администрирования; основные действия не требуют ручного ввода slash-команд.
- Web-panel получила супер-админский раздел безопасных обновлений: отдельный systemd-unit, блокировка параллельных запусков, проверка release SHA-256, rollback snapshot и post-update health-check.
- Добавлен компактный `/api/bot/snapshot` для быстрых сводок без тяжёлого browser-only обогащения.
- Telegram-бот получил параллельные запросы к панелям, кэш snapshots на 5 секунд, inline-меню и компактный список клиентов.
- Статические JS/CSS-файлы web-панели теперь отдаются с gzip при поддержке клиента.
- IPv6 leak-block теперь применяется и к split-tunnel профилям: `::/0` блокирует обход IPv6/WebRTC без выдачи клиентского IPv6-адреса.
- Telegram-бот теперь учитывает состояние `awg-quick` и свежесть handshake: показывает `degraded/down` вместо ложного «ok» и отправляет админу уведомления о падении и восстановлении панели.
- Кнопка «Обновить» теперь корректно обрабатывает ответ Telegram `message is not modified`: текущая карточка остаётся на месте и не удаляется/не дублируется.
- Статус панели больше не становится оранжевым из-за выключенных телефонов/ноутбуков: число подключённых клиентов отображается информативно, а тревога возникает только при ошибке API или неактивном VPN-сервисе.
- Служебный токен GaulleBot больше не отображается как владелец каждого клиентского конфига: ownership-бейджи и фильтры показывают только пользовательские токены.
...
- Миграция tunnel subnet больше не сканирует исторические health-history, GeoIP-базы и логи; plan-only не зависает на больших telemetry-файлах.
- Обновлена безопасная процедура Germany subnet migration `10.9.9.1/24 -> 10.9.10.1/24`.
