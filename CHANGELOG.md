...
- Telegram-бот переработан в минимальный интерфейс: главное меню содержит только устройства, добавление, избранное, подключение, серверы и доступ; карточка устройства разделяет QR/`.conf`, дополнительную информацию и настройки. Нагрузка, готовность VPN, DNS/AdGuard, логи, сетевые тесты и обслуживание перенесены в веб-панель/Mini App.
