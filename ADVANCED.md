...
- [🆕 Третья линия AmneziaWG для self-hosted](#awg3-adv)
  ...
  - [Совместимость и безопасные значения по умолчанию](#awg3-not-yet-adv)
...
В конце июля 2026 команда Amnezia выпустила **AmneziaWG 3.0**, а 12 августа внутри той же линии вышла **3.1** (тег `v3.1.20260812`). Установщик не угадывает поддержку по версии ядра: для выбранного протокола он проверяет реальную способность загруженного модуля принять и вернуть параметры через capability probe.
...
|---|---|---|
| x86_64 / ARM64 / armhf, есть пакет или DKMS | PPA либо предсобранный пакет | определяется `AWG_PROTOCOL_VERSION` и probe |
| старое ядро, контейнер или ограниченный хост | pinned DKMS либо userspace fallback | capability probe, без kernel-version gate |

Готовые ARM-пакеты выпускаются для части Raspberry Pi и облачных образов; при отсутствии совпадения используется DKMS. В любом случае окончательное решение принимает probe, а не название пакета.

Номер ядра сам по себе не является ограничением AWG 3.1. Если модуль собирается и probe проходит, протокол можно использовать; если probe не проходит, установщик сообщает причину и не выдаёт 3.1-конфиг.

На ARM установщик сначала пробует предсобранный пакет, затем использует DKMS. После загрузки capability probe проверяет именно доступные поля, поэтому пакет и архитектура не считаются доказательством поддержки 3.1.
...
>
> На практике ориентируйтесь на `manage profile status`, а не на `uname -r`, версию пакета или `modinfo`. При несовпадении tools/module рендеринг 3.1 блокируется, чтобы не выдать неработающий конфиг.
>
> Если probe сообщает о неподдерживаемом модуле, проверьте загруженный модуль и DKMS/headers, затем повторите установку или выберите совместимый `--awg-version=2.0`. Не переключайте действующий `awg0` вручную.
...
### Совместимость и безопасные значения по умолчанию

Профиль AWG 3.1 генерируется только после успешной capability-проверки и передаётся целиком серверу и клиентам. Профили 1.5/2.0/3.0 остаются доступны для старых клиентов; версии параметров нельзя смешивать:

- **`HeaderProtectionKey`**, `S1`-`S4`, `H1`-`H4` и `RandomTrailers` должны быть одинаково поддержаны и согласованы у сервера и клиентов. Старые приложения подключайте профилем 2.0.
- **`DisableCookies`** по умолчанию выключен: включайте его только осознанно, потому что он отключает защиту от flood-рукопожатий.
- Если клиент не поддерживает 3.1, не пытайтесь «смягчить» его конфигом: выпустите совместимый профиль 2.0 и не смешивайте поля между версиями.
...
<details>
<summary><strong>Минимальный awg0.conf для AWG 2.0 (legacy, если настраивается вручную)</strong></summary>

> **Важно для AWG 3.1:** этот пример намеренно показывает legacy-конфигурацию 2.0. Для новой установки используйте `install_amneziawg.sh --awg-version=3.1`: профиль создаётся генератором и принимается только после capability probe/read-back. Не переносите значения S3/S4 из этого примера в профиль 3.1 вручную.
...
### Capability probe вместо проверки версии ядра

Старые рецепты ниже ошибочно связывали поддержку 3.x с порогом ядра 6.7. Это не является контрактом AmneziaWG: совместимость определяется фактическим round-trip capability probe через временный интерфейс.

Для диагностики используйте `manage profile status`: он отдельно показывает выбранную версию профиля, валидность файла и результат probe. Если probe не проходит, рендерер работает fail-closed и не добавляет 3.1-поля в конфиг другого протокола.

Если probe не проходит, не меняйте действующий `awg0` на месте: сохраните backup, исправьте путь модуля/DKMS или выберите совместимый профиль 2.0. Включение 3.1 требует согласованных серверного и клиентских конфигов.
