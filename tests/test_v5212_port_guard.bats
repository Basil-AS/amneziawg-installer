...
_setup_stubs() {
    ...
    generate_keypair() {
        printf 'ROTATED_PRIVATE' > "$KEYS_DIR/${1}.private"
        printf 'ROTATED_PUBLIC' > "$KEYS_DIR/${1}.public"
    }
    replace_peer_credentials() { return 0; }
    generate_firewall_scripts() { return 0; }
    sync_clients_hosts() { return 0; }
    ...
    export -f get_server_public_ip _ensure_server_public_key generate_qr \
        generate_vpn_uri generate_qr_vpnuri generate_keypair replace_peer_credentials \
        generate_firewall_scripts sync_clients_hosts apply_config load_awg_params render_client_config
}
...
@test "v5.21.2: generate/regenerate render the sanitized port, not raw AWG_PORT (RU+EN)" {
    for f in awg_common.sh awg_common_en.sh; do
        ...
        # All three client render paths (add, refresh, rotate/regen) must pass
        # the sanitized "$_cport", never the raw
        # "${AWG_PORT}".
        run grep -cF 'render_client_config "$name" "$client_ip" "$client_privkey" "$server_pubkey" "$endpoint" "$_cport" "$client_ipv6"' "$src"
        [ "$output" -eq 3 ] || { echo "$f: expected 3 sanitized render calls, found $output"; return 1; }
