...
@test "web app exposes server health and network tester UI" {
    ...
    grep -qF 'grid-template-columns:repeat(6,minmax(max-content,1fr))' "$css"
    grep -qF '.summary-cards.no-provider{grid-template-columns:repeat(5,minmax(max-content,1fr))}' "$css"
