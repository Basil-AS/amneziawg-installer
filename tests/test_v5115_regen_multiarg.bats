...
@test "fork uses BAS release version 5.19.2-bas.8" {
    ...
    for f in install_amneziawg.sh install_amneziawg_en.sh manage_amneziawg.sh manage_amneziawg_en.sh; do
        run grep -E 'SCRIPT_VERSION="5\.19\.2-bas\.7"' "$BATS_TEST_DIRNAME/../$f"
        [ "$status" -eq 0 ]
    done
    ...
    [ "$status" -eq 0 ]
    run grep -Fx '5.19.2-bas.8' "$BATS_TEST_DIRNAME/../VERSION"
    [ "$status" -eq 0 ]
}
