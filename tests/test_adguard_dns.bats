...
@test "installer renders curated AdGuard Home config and validates it with binary" {
    ...
    grep -qF 'v6_offset = 0x100 if os.environ.get("AWG_IPV6_MODE_EFFECTIVE") == "ndp" else 1' "$installer"
    grep -qF 'bind_hosts.append(str(v6_net.network_address + v6_offset))' "$installer"
