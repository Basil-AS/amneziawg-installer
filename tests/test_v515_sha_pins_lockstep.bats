...
_expected_sha() {
    ...
    # The fork defaults to its moving main branch, so its pins must match the
    # helper bytes in this checkout rather than an upstream release tag.
    if grep -qE '^AWG_BRANCH="\$\{AWG_BRANCH:-main\}"' "$ROOT/$installer"; then
...
_pinned_sha() {
    ...
    # The fork stores all asset pins in AWG_ASSET_SHA256 rather than scalar
    # COMMON_SCRIPT_SHA256/MANAGE_SCRIPT_SHA256 variables.
    local asset
    case "$2:$1" in
        COMMON:install_amneziawg.sh) asset=awg_common.sh ;;
        MANAGE:install_amneziawg.sh) asset=manage_amneziawg.sh ;;
        COMMON:install_amneziawg_en.sh) asset=awg_common_en.sh ;;
        MANAGE:install_amneziawg_en.sh) asset=manage_amneziawg_en.sh ;;
        *) asset= ;;
    esac
    if [ -n "$asset" ]; then
        awk -v key="$asset" '$0 ~ "^[[:space:]]*\\[\\\"" key "\\\"\\]=\\\"[0-9a-f]{64}\\\"" {
            line=$0
            sub("^[[:space:]]*\\[\\\"" key "\\\"]\\]=\\\"", "", line)
            sub("\\\".*$", "", line)
            print line
