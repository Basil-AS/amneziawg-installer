...
# helper scripts over the network and verify them against hardcoded pins in
# AWG_ASSET_SHA256. If any pin drifts from the
...
@test "RU installer COMMON pin matches awg_common.sh" {
    ...
    pinned=$(grep -oP '\["awg_common\.sh"\]="\K[0-9a-f]{64}' "$BATS_TEST_DIRNAME/../install_amneziawg.sh")
    [ "$actual" = "$pinned" ] || {
...
@test "RU installer MANAGE pin matches manage_amneziawg.sh" {
    ...
    pinned=$(grep -oP '\["manage_amneziawg\.sh"\]="\K[0-9a-f]{64}' "$BATS_TEST_DIRNAME/../install_amneziawg.sh")
    [ "$actual" = "$pinned" ] || {
...
@test "EN installer COMMON pin matches awg_common_en.sh" {
    ...
    pinned=$(grep -oP '\["awg_common_en\.sh"\]="\K[0-9a-f]{64}' "$BATS_TEST_DIRNAME/../install_amneziawg_en.sh")
    [ "$actual" = "$pinned" ] || {
...
@test "EN installer MANAGE pin matches manage_amneziawg_en.sh" {
    ...
    pinned=$(grep -oP '\["manage_amneziawg_en\.sh"\]="\K[0-9a-f]{64}' "$BATS_TEST_DIRNAME/../install_amneziawg_en.sh")
    [ "$actual" = "$pinned" ] || {
