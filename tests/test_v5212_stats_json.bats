...
_assert_sanitized() {
    ...
    # The fork extends upstream stats with IPv6, endpoint and P2P fields.
    printf '%s' "$output" | jq -e '
        all(.[]; keys == ["endpoint","ip","ipv6","last_handshake","name","p2p_ports","rx","status","status_code","tx"])' >/dev/null \
