...
- Fixed portable SHA-manifest and English pin checks in release preflight.
...
- VPN clients can no longer reach local web-panel addresses on TCP 80/443 or the direct panel port through AWG forwarding.
- A panel domain cannot be reused as the VPN endpoint.

## [5.19.2-bas.7] - 2026-08-21

### Fixed

- `INSTALL_SUMMARY.txt` now shows the configured panel domain instead of a stale IP from the old endpoint.

## [5.19.2-bas.6] - 2026-08-21

### Fixed

- The Telegram bot installer now ships regression tests and runs a compile/unit preflight before restarting systemd.
- Reinstalling without a new `--ssh-key` preserves the existing SSH identity paths, so the `fin.charles.men` and `ger.charles.men` tunnels keep working.
...
- Safe installer integration, a systemd deployment unit, and bilingual documentation.
